...
That means the exported static site is committed to the repository, inside `docs/`. Whatever sits in `docs/` on `main` is what visitors see. There is no build step running on GitHub's side — the build happens locally and the result is pushed.
...
From an up-to-date `main` branch with a clean working tree:
...
The push is deliberately a separate step, so you can inspect the generated diff before making it public.
...
| 4 | `npm run release:gp:commit` | Runs `git add docs/` and commits with the message `release: new features`. |

Each step can be run on its own, which is useful when debugging a broken release.

## Verifying before you push

After running the release, check the export locally instead of trusting the diff:
...
git show --stat       # review what the release commit touched
```

Once pushed, GitHub Pages usually publishes within a minute. The deployment status is visible in the repository's **Settings → Pages** section and under the **Deployments** tab.
